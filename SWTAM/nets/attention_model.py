...
class AttentionModel(nn.Module):
    ...
    def _inner(self, input):
        ...
        if self.is_vrp:
            mask=torch.zeros(batch_size,1,21)
        else:
            mask=torch.zeros(batch_size,1,20)


        while not (self.shrink_size is None and state.all_finished()):
            if self.is_vrp:
                embeddings, init_context = self.embedder(self._init_embed(input), mask=mask, is_vrp=True)
            else:
                # keep the first node for tsp
                if i>0:
                    mask_tmp = mask ^ mask_first
                    embeddings, init_context = self.embedder(input, mask=mask_tmp)
                    mask_tmp = (~mask_tmp).view(batch_size, 20 ,1).repeat(1, 1, 128).float()
                    init_context = torch.sum(embeddings * mask_tmp, dim = 1) / (21 - i)
                else:
                    embeddings, init_context = self.embedder(input,mask=mask)
                    init_context = torch.sum(embeddings, dim = 1) / 20

            # Compute keys, values for the glimpse and keys for the logits once as they can be reused in every step
            fixed = self._precompute(embeddings, init_context)

            log_p, mask = self._get_log_p(fixed, state)
            if not self.is_vrp and i==0:
                mask_first = mask
            # print (mask.size())
